...
?>
		...
		<table class="table">
    		...
    		<tr>
  			...
  			<tbody>
  			<?php
  				foreach ($wanted_thread_lists as $wanted_thread_list) { ?>

  				<tr>
      			<td><?php echo $wanted_thread_list['User']['name']; ?></td>
            <td>
      			<?php echo $wanted_thread_list['Wanted_thread_list']['thread']; ?>
            </td>
      			<td>
            <?php echo $wanted_thread_list['Wanted_thread_list']['created']; ?>
            </td>
      			<td>
      				<?php
            if(!$wanted_thread_list['User']['block_flg']){
              echo $this->Form->postLink(
                '<button type="button" class="btn btn-danger">ロック</button>',
                array('action' => '/lock_unlock', $wanted_thread_list['User']['id']),
