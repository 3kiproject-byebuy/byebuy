...
//----------------------------------------------------------------------------------------------------------------------------------//?>
			...
			//商品詳細 ?>
				...
				if (!is_null($self))
					...
					<?php } ////③trade_person_use_idが0では期限内だったらコメント入力を表示　終了?>
...
<?php //もしログインユーザーidが商品のuser_idと同一で、取引が成立していない場合（ログインユーザーが出品者だった場合） ?>
<?php if($sellinglists[0]['Selling_list']['user_id'] == $self['id'] && $sellinglists[0]['Selling_list']['status'] != 2) { ?>
	...
	<?php 
	//もしtrade_person_use_idが0 and 締め切りを過ぎていない and ステータスが2でなく and ログインユーザー and ログインユーザー = 商品出品者 だったらコメント表示
	if ($sellinglists[0]['Selling_list']['trade_person_use_id'] == 0 && strtotime($sellinglists[0]['Selling_list']['deadline']) > strtotime($current_date) && $sellinglists[0]['Selling_list']['status'] != 2 && !is_null($self) && $sellinglists[0]['Selling_list']['user_id'] == $self['id']) { ?>
	...
	<?php } //もしtrade_person_use_idが0もしくは期限がきていなくて、商品出品者=ログインユーザーだったら 終了
